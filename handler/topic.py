...
from lib.sendmail import notice
...
class NodeTopicsHandler(BaseHandler):
    def get(self, node_slug, template_variables = {}):
        node_exist_status = self.node_model.get_node_by_node_slug(node_slug)
        if node_exist_status is not None:
            user_info = self.current_user
            page = int(self.get_argument("p", "1"))
            template_variables["user_info"] = user_info
            if(user_info):
...
class ViewHandler(BaseHandler):
    ...
    def post(self, template_variables = {}):
        ...
        if not self.current_user["uid"] == topic_info["author_id"]:
            ...
            user_info = self.user_model.get_user_by_uid(self.current_user["uid"])
            template_variables["trigger_user"] = user_info['username']
            topic_info = self.topic_model.get_topic_by_topic_id(form.tid.data)
            template_variables["involved_topic"] = topic_info['title']
            reply_email = self.user_model.get_user_by_uid(topic_info["author_id"])
            mail_content = self.render_string("user/reply_mail.html", **template_variables)
            try: 
                reply(mail_content,reply_email['email'])
            except Exception,e:
                print (e)
                return false
        ...
        for username in set(find_mentions(form.content.data)):
            ...
            topic_info = self.topic_model.get_topic_by_topic_id(form.tid.data)
            template_variables["involved_topic"] = topic_info['title']
            notice_email = self.user_model.get_user_by_uid(mentioned_user["uid"])
            mail_content = self.render_string("user/notice_mail.html", **template_variables)

            try: 
                notice(mail_content,notice_email['email'])
            except Exception,e:
                print (e)
                return false
...
#        self.members =  
    ...
    #    template_variables["members"] = self.user_model.get_users_by_latest(num = 49)
        template_variables["members"] =[ ('Azure','http://azure.microsoft.com'),('Hp','http://www.hpcloud.com/'),\
                                         ('Aws','http://aws.amazon.com/cn'),('Google','https://cloud.google.com/'),\
                                         ('Rackspace','http://www.rackspace.com'),('Dotcloud','https://www.dotcloud.com/'),\
                                         ('腾讯云','http://www.qcloud.com/'),('阿里云','http://www.aliyun.com/'),\
                                         ('Ucloud','http://www.ucloud.cn/'),('Unitedstack','https://www.ustack.com/'),\
                                         ('华为云','http://www.hwclouds.com/'),('青云','http://www.qingcloud.com/'),\
                                         ('京东云','http://jae.jd.com/'),('美团云','https://mos.meituan.com/'),\
                                         ('刻通云','http://keytonecloud.com/'),('金山云','http://www.ksyun.com/'),\
                                         ('世纪互联','http://www.viacloud.com.cn/'),('浪潮','http://cloud.inspur.com/'),\
                                         ]
        template_variables["paas"] = [ ('Bluemix','https://console.ng.bluemix.net/'),('搜狐云景','http://cloudscape.sohu.com/'),\
                                       ('Openshift','https://www.openshift.com'),('SAE','http://sae.sina.com.cn/'),\
                                       ('七牛','http://www.qiniu.com/'),('Leancloud','https://leancloud.cn/'),\
                                     ]
